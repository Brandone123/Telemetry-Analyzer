...
export class XlsxTemplate {
  ...
  /**
   * Insert a numeric cell into existing rows for a brand-new column that has no
   * pre-existing cells. Unlike updateColumnValues (which only rewrites existing
   * <c> elements), this appends a new <c> element inside each matching <row>.
   * Cells are appended before </row> — Excel tolerates out-of-order columns on
   * open and re-sorts them.
   */
  async insertColumnValues(
    sheetName: string,
    columnLetter: string,
    valuesByRow: Map<number, number>,
  ): Promise<void> {
    if (valuesByRow.size === 0) return;
    let xml = await this.getSheetXml(sheetName);
    xml = xml.replace(
      /<row\s+r="(\d+)"([^>]*)>([\s\S]*?)<\/row>/g,
      (whole, rowStr: string, attrs: string, content: string) => {
        const rowNum = parseInt(rowStr, 10);
        if (!valuesByRow.has(rowNum)) return whole;
        const val = valuesByRow.get(rowNum)!;
        const newCell = `<c r="${columnLetter}${rowNum}"><v>${val}</v></c>`;
        return `<row r="${rowStr}"${attrs}>${content}${newCell}</row>`;
      },
    );
    this.setSheetXml(sheetName, xml);
  }
  /**
   * Single-pass zero-out of all data cells that belong to a given set of
   * columns, within the row range [firstDataRow..lastDataRow].
   * Much faster than calling updateColumnValues per-column for month resets.
   */
  async clearColumnsDataRows(
    sheetName: string,
    columns: Set<string>,
    firstDataRow: number,
    lastDataRow: number,
    ...
    xml = xml.replace(
      ...
      (match: string, col: string, rowStr: string, attrs: string) => {
        if (!columns.has(col)) return match;
        const rowNum = parseInt(rowStr, 10);
        if (rowNum < firstDataRow || rowNum > lastDataRow) return match;
        const cleanAttrs = attrs.replace(/\s+t="[^"]*"/g, "");
        return `<c r="${col}${rowNum}"${cleanAttrs}/>`;
      },
    );
    this.setSheetXml(sheetName, xml);
  }
